...
{
	iowrite8(position->x>>8, dev.virtbase );
	iowrite8(position->x & 0xFF, dev.virtbase+1 );
	iowrite8(position->y>>8, dev.virtbase+2 );
	iowrite8(position->y & 0xFF, dev.virtbase+3 );
...
static long vga_ball_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
	...
	case VGA_BALL_WRITE_POSITION:
		if (copy_from_user(&vla.position,(vga_ball_position_t *) arg,
	...
	case VGA_BALL_READ_POSITION:
		...
		if (copy_to_user((vga_ball_position_t *) arg, &vla,
				 sizeof(vga_ball_position_t)))
			return -EACCES;
		break;
